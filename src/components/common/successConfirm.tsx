...
export const SuccessConfirm: React.FC<SuccessConfirmProps> = ({
  ...
  return (
    <div className="flex h-full w-full flex-col items-center justify-center space-y-4 text-center">
      ...
      <p className="body-sm text-base-content/70 mt-4">{description}</p>

      {children && <div className="w-full max-w-125 pt-2">{children}</div>}

      <Button
        onClick={onButtonClick}
        variant="primary"
        size="md"
        className={cn('rounded-xl btn-block h-12 md:h-15 max-w-125 mt-2 md:mt-6', buttonClassName)}
      >
        {buttonText}
      </Button>
    </div>
